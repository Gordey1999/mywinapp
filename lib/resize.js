...
async function getResized(absPath) {
    const cachePath = getFileCachePath(absPath);

    if (fs.existsSync(cachePath)) {
        const imageStats = fs.statSync(absPath);
        const cacheStats = fs.statSync(cachePath);

        if (imageStats.mtime.toISOString() <= cacheStats.mtime.toISOString()) {
...
function makeCacheUrl(cachePath, dateCreate) {
    return cachePath + '?r=' + dateCreate;
}
