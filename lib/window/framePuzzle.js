...
function concatWithNearest(win, width, height) {
    ...
    for (let j = 0; j < windows.length; j++) {
        ...
        if (Math.abs(Math.abs(xDist) - width) + Math.abs(yDist) < minDist) {
            offsetY = yDist;
            offsetX = Math.abs(xDist) - width;
            if (xDist < 0) offsetX = -offsetX;

            minDist = Math.abs(Math.abs(xDist) - width) + Math.abs(yDist);
        }
        if (Math.abs(Math.abs(yDist) - height) + Math.abs(xDist) < minDist) {
            offsetX = xDist;
            offsetY = Math.abs(yDist) - height;
            if (yDist < 0) offsetY = -offsetY;

            minDist = Math.abs(Math.abs(yDist) - height) + Math.abs(xDist);
        }
    }

    if (minDist < settings.maxOffset * 2) {
...
function windowSolved(win) {
    win.webContents.send('framePuzzleSolved', settings.mute);
