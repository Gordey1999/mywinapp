...
winManager.on('filesDragStart', async (data, src, selected) => {
    ...
    const icon = images.length > 0 ? path.join(src, images[0]) : path.join(settings.cacheDir, 'drag_icon_default.png');
...
async function buildDragIcon(src, count) {
    ...
    await sharp(src)
        ...
        .composite([
        {
            input: {
                text: {
                    text: '<span color="white" bgcolor="#302d57">(' + count + ')</span>',
                    font: 'system-ui',
                    width: 100,
                    height: 40,
                    align: 'center',
