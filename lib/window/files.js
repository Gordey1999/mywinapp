...
function readFiles(dirPath, sort = 'nameAsc') {
    ...
    if (sort === 'nameAsc') {
        ...
        for (const file of files) {
            file.birthtime = fs.statSync(file.src).birthtime.getTime();
        }
...
winManager.handle('filesItemList', async (data, src) => {
    ...
    for (let dir of dirs) {
        ...
        const images = dirFiles.filter(file => file.type === 'image' || file.type === 'gif');
